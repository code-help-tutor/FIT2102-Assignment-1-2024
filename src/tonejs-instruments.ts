...
interface SampleLibraryOptions {
    instruments?: string[];
    baseUrl?: string;
    onload?: () => void;
    ext?: string;
...
interface SampleLibraryType {
    ...
    load: (arg?: SampleLibraryOptions) => { [key: string]: Tone.Sampler };
    [key: string]: any; // Allow for dynamic keys representing instruments
}

const calcMinBy = (length: number) => {
    if (length >= 49) return 6;
    if (length >= 33) return 4;
...
export const SampleLibrary: SampleLibraryType = {
    ...
    setExt: function (newExt: string): void {
    ...
    load: function (arg?: SampleLibraryOptions): {
        ...
        const rt: { [key: string]: Tone.Sampler } = {};
        // let newT: InstrumentSamples = null;
        ...
        if (Array.isArray(t.instruments)) {
            for (let i = 0; i <= t.instruments.length - 1; i++) {
                ...
                if (this.minify || t.minify) {
                    const minBy = calcMinBy(Object.keys(newT).length);
                    const filtered = Object.keys(newT).filter(
                        (_, i) => i % minBy !== 0,
                    );
            ...
            const newT = this[t.instruments as string];
            ...
            const rt: { [key: string]: Tone.Sampler } = {};
            rt[t.instruments] = new Tone.Sampler(newT, {
